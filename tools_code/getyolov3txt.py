...
testpic = 'E:/hnumedical/4C_ABP_new/to_test_txtjson/to_test_yolov3.txt'
  ...
  for cur_file in dir_list:
    ...
    if os.path.isfile(path) and cur_file == 'annotations.json':
        ...
        for x in annotations:
            ...
            height = sp[0]
            width = sp[1]
            testpictxt.writelines(file_dir + '/' + str(x))
            for i in range(len(classname)):
                if classname[i] in class_mapping_1:
                    ...
                    testpictxt.writelines(' '+ str(xmin)+','+str(ymin)+','+str(xmax)+','+str(ymax)+','+str(classes.index(class_mapping_1[classname[i]])))
            testpictxt.writelines('\n')
    if os.path.isdir(path):
        list_dir(path)
list_dir('E:/hnumedical/4C_ABP_new/to_test/')